...
type CameraForm = { name: string; type: 'gate' | 'zone'; stream_url: string };
...
export default function CreateParking({ isPremium }: Props) {
    ...
    // Erreurs serveur (retournées par Inertia après validation Laravel)
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const fieldsToValidate = ['name','city','latitude','longitude','total_spots','price_per_hour','cancel_time_limit'];
        fieldsToValidate.forEach(f => validateField(f, String(fields[f as keyof typeof fields])));

        if (!photo) {
            setClientErrors(p=>({...p,photo:'Photo is required.'}));
            return;
        }

        if (Object.keys(clientErrors).length > 0) return;

        // ✅ Construire FormData manuellement — seul moyen fiable avec fichiers + JSON
        ...
        // Annotation + slots — seulement si Premium
                        ...
                        <div className="space-y-3">
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground mt-0.5">
                                    Displayed on parking cards and detail page. Max 4MB.
                                </p>
                            </div>
                        ...
                        {isPremium ? (
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <p className="text-xs text-muted-foreground mt-0.5">
                                        Top-view photo or floor plan for spot definition. Max 10MB.
                                    </p>
                                </div>
                                ...
                                {annotPreview && !slots.length && (
                                    <div className="relative inline-block">
                                        <img src={annotPreview} alt="Annotation preview"
                        ...
                        <div className="space-y-4 rounded-xl border-2 border-indigo-200 bg-indigo-50/30 dark:border-indigo-900 dark:bg-indigo-950/20 p-5">
                            <div className="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground mt-1">
                                        Click 4 corners of each spot on your floor plan.
                                    </p>
                                </div>
                                {slots.length > 0 && (
                            ...
                            {slots.length > 0 && (
                                <div className="flex items-center gap-2 rounded-lg bg-green-50 border border-green-200 px-3 py-2 text-sm text-green-700 dark:bg-green-950/30 dark:border-green-900 dark:text-green-400">
                                    ...
                                    <span>
                                        <strong>{slots.length} spot{slots.length>1?'s':''}</strong> defined — the annotated image will be generated on save.
                                    </span>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="space-y-4 rounded-lg border p-5 bg-muted/10">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground mt-1">
                                    Add IP cameras or DroidCam URLs to monitor your parking.
                                </p>
                            </div>
                        ...
                        <Button type="submit" className="flex-1" disabled={processing}>
                            {processing
                                ...
                                : isPremium && slots.length > 0
                                    ? `Save with ${slots.length} Annotated Spot${slots.length>1?'s':''}`
                                    : 'Add Parking'
                            }
                        </Button>
